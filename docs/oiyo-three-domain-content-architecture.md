...
작성일: 2026-06-03  
수정일: 2026-06-05 (도메인별 콘텐츠 경계 재확정)
...
세 도메인은 같은 주제를 다룰 수 있지만, 같은 목적의 문서를 세 곳에 만들지 않는다.

| 도메인 | 주 역할 | MBTI 예시 | 콘텐츠 톤 |
|---|---|---|---|
| `oiyo.net` (`oiyo-astro`) | 실행, 결과, 짧은 선택 안내 | `/ko/mbti/test` | 바로 테스트하고 해석하는 제품 문장 |
| `blog.oiyo.net` | 장문 해설, 맥락, SEO 매거진 | `/ko/mbti-career-test`, `/ko/magazine-mbti-career-psychology` | 읽으면서 이해하는 설명형 글 |
| `wiki.oiyo.net` | 정의, 유형 사전, 참조 | `/ko/meaning-of-mbti`, `/ko/mbti-dict-intj` | 짧고 구조화된 사전 문장 |

따라서 `oiyo-astro`의 콘텐츠 강화는 블로그 글을 복제하거나 위키 정의를 다시 쓰는 것이 아니다. 테스트 전후에 필요한 최소 안내, 결과 해석 기준, 안전 문구, 관련 행동 링크를 추가하는 방식으로 진행한다.
