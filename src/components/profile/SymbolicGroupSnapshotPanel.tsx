...
export default function SymbolicGroupSnapshotPanel({ initial, initialSnapshot, locale }: { initial?: Array<{ label: string; profile: SymbolicComparisonProfile }>; initialSnapshot?: SymbolicGroupSnapshot; locale: string }) {
  ...
  return <section className="mt-8 rounded-[2rem] border border-lime-200 bg-[#f7f8ed] p-4 sm:p-7" aria-labelledby="group-title">
    <div className="text-center"><Network className="mx-auto h-7 w-7 text-green-800" /><h2 id="group-title" className="mt-2 text-2xl font-black text-green-950">{copy.title}</h2><p className="mt-2 text-sm leading-6 text-stone-600">{copy.sub}</p></div>
    {snapshot && <>
      <div className="mt-5 flex gap-2 overflow-x-auto pb-1">{(Object.keys(LENS[lang]) as CompatibilityLensId[]).map((id) => <button key={id} onClick={() => setLens(id)} type="button" className={`min-h-11 shrink-0 rounded-full px-4 text-xs font-black ${lens === id ? "bg-green-800 text-white" : "border border-lime-200 bg-white text-green-900"}`}>{LENS[lang][id]}</button>)}</div>
      <div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => setView("star")} className={`min-h-11 rounded-xl text-xs font-black ${view === "star" ? "bg-lime-200 text-green-950" : "bg-white text-stone-600"}`}><Star className="mr-1 inline h-4 w-4" />{copy.star}</button><button type="button" onClick={() => setView("all")} className={`min-h-11 rounded-xl text-xs font-black ${view === "all" ? "bg-lime-200 text-green-950" : "bg-white text-stone-600"}`}><Network className="mr-1 inline h-4 w-4" />{copy.all}</button></div>
      <div className="mt-4 aspect-square max-h-[32rem] w-full overflow-hidden rounded-3xl border border-lime-100 bg-white" role="img" aria-label={`${copy.title}: ${LENS[lang][lens]}`}><svg viewBox="0 0 100 100" className="h-full w-full">
        {edges.map((edge) => { const from = position(edge.from); const to = position(edge.to); return <line key={`${edge.from}-${edge.to}`} x1={from.x} y1={from.y} x2={to.x} y2={to.y} stroke="#84a844" strokeWidth="0.7"><title>{edge.relation}</title></line>; })}
    ...
    {!snapshot && <p className="mt-5 rounded-2xl bg-white px-4 py-3 text-center text-sm font-bold text-green-900">{copy.need}</p>}
    <div className="mt-5 space-y-2">{participants.map((person) => <div key={person.id} className="flex min-h-12 items-center gap-3 rounded-2xl bg-white px-4"><button type="button" onClick={() => setCenterId(person.id)} className="min-w-0 flex-1 truncate text-left text-sm font-black text-green-950">{person.id === centerId ? "◎ " : "○ "}{person.label}</button>{participants.length > 2 && !initialSnapshot && <button aria-label={`${person.label} ${copy.remove}`} type="button" onClick={() => removeParticipant(person.id)} className="h-11 w-11 text-stone-400"><Trash2 className="mx-auto h-4 w-4" /></button>}</div>)}</div>
    {!initialSnapshot && participants.length < 10 && <div className="mt-5 grid gap-3 rounded-3xl border border-lime-200 bg-white p-4 sm:grid-cols-[1fr_2fr_auto]"><input aria-label={copy.alias} placeholder={copy.alias} value={alias} maxLength={24} onChange={(event) => setAlias(event.target.value)} className="h-12 rounded-2xl border border-stone-200 px-4 text-base" /><input aria-label={copy.link} placeholder={copy.link} value={link} onChange={(event) => setLink(event.target.value)} className="h-12 min-w-0 rounded-2xl border border-stone-200 px-4 text-base" /><button type="button" onClick={() => void addFriend()} className="min-h-12 rounded-2xl bg-green-800 px-5 text-sm font-black text-white"><Plus className="mr-1 inline h-4 w-4" />{copy.add}</button></div>}
