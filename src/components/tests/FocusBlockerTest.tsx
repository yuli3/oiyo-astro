...
const categoryInfo: Record<BlockerCategory, {
  ...
  taskSwitching: { emoji: "🔀", color: "#5B915F", ko: { title: "업무 전환 방해", action: "타임블로킹 및 우선순위 관리" }, en: { title: "Task Switching", action: "Time-blocking and priority management" }, ja: { title: "タスク切り替えの妨害", action: "タイムブロッキングと優先順位管理" }, zh: { title: "任务切换干扰", action: "使用时间块，并管理优先级" }, fr: { title: "Changement de tâche", action: "Utiliser le time blocking et gérer les priorités" }, es: { title: "Cambio de tareas", action: "Usar bloques de tiempo y gestionar prioridades" } },
...
export default function FocusBlockerTest({ locale: localeProp }: Props) {
  ...
  useRecordFinishedTest({ testId: "focus-blocker", title: "FocusBlockerTest", finished: Boolean(showResult) });
