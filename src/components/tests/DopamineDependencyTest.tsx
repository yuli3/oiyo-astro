...
const RESULTS: Record<Level, Record<Locale, { title: string; color: string; bg: string; symptoms: string[]; tips: string[] }>> = {
  ...
  critical: {
    ko: { title: '심각한 의존 ⚠️', color: 'text-green-700', bg: 'bg-green-50 border-green-200', symptoms: ['즉각적 자극 없이 일상 기능 어려움', '디지털 제한 시 심한 금단 증상', '수면·식사 등 기본 생활 리듬 무너짐', '현실 관계보다 디지털 관계 우선시'], tips: ['전문가 상담(행동 중독 전문의) 권장', '1주일 완전한 디지털 디톡스 시도', '주변 사람들의 지지 적극 활용', '기저 불안·우울증 확인 필요'] },
    en: { title: 'Critical Dependency ⚠️', color: 'text-green-700', bg: 'bg-green-50 border-green-200', symptoms: ['Difficulty functioning without instant stimulation', 'Severe withdrawal when digital access is limited', 'Basic life rhythms (sleep, eating) disrupted', 'Prioritizing digital relationships over real ones'], tips: ['Recommend consulting a behavioral addiction specialist', 'Attempt a full week-long digital detox', 'Actively utilize support from people around you', 'Check for underlying anxiety or depression'] },
    ja: { title: '深刻な依存 ⚠️', color: 'text-green-700', bg: 'bg-green-50 border-green-200', symptoms: ['即時刺激なしに日常機能が困難', 'デジタル制限時に重篤な禁断症状', '睡眠・食事など基本生活リズムが崩れている', 'リアルな関係よりデジタル関係を優先'], tips: ['専門家相談（行動依存専門医）を推奨', '1週間完全デジタルデトックスを試みる', '周囲の人のサポートを積極的に活用', '基礎的な不安・うつ病の確認が必要'] },
