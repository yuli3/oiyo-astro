...
export default function EmpathyTest({ locale: localeProp }: Props) {
  ...
  function pick(score: number) {
    if (answers.length === 0) gaEvent("test_started", { test_id: "empathy", instrument_version: "empathy-oiyo-12-v1" });
      ...
      <div className="space-y-6">
        ...
        <ResultNextSteps
          ...
          links={[
            ...
            { href: `/${locale}/ontology/personality/`, label: locale === 'ko' ? '🧭 성격 지도' : locale === 'ja' ? '🧭 性格マップ' : locale === 'zh' ? '🧭 人格地图' : locale === 'fr' ? '🧭 Carte de personnalité' : locale === 'es' ? '🧭 Mapa de personalidad' : '🧭 Personality map' },
          ]}
        />
