...
const LABELS: Record<Locale, {
  ...
  cn: { title: '打字速度測試', subtitle: '您在1分鐘內能打多快多準確？', start: '開始', restart: '重新測試', stop: '停止', wpm: '每分鐘單詞數', cpm: '每分鐘字符數', accuracy: '準確率', time: '剩餘時間', errors: '錯誤', result: '結果', typeHere: '在下方輸入文字', waiting: '按開始啟動計時器', finished: '完成！', level: (wpm) => wpm >= 80 ? '打字高手 ⚡' : wpm >= 50 ? '中級 👍' : wpm >= 30 ? '初學者 📝' : '需要練習 💪' },
  zh: { title: '打字速度测试', subtitle: '您在1分钟内能打多快多准确？', start: '开始', restart: '重新测试', stop: '停止', wpm: '每分钟单词数', cpm: '每分钟字符数', accuracy: '准确率', time: '剩余时间', errors: '错误', result: '结果', typeHere: '在下方输入文字', waiting: '按开始启动计时器', finished: '完成！', level: (wpm) => wpm >= 80 ? '打字高手 ⚡' : wpm >= 50 ? '中级 👍' : wpm >= 30 ? '初学者 📝' : '需要练习 💪' },
}
...
export default function TypingSpeedTest({ locale }: Props) {
  ...
  useRecordFinishedTest({ testId: "typing-speed", title: "TypingSpeedTest", finished: status === 'done' });
