...
// 수준 문턱은 Big Five 와 같은 65/35 를 쓴다. 이 검사만 다른 문턱을 두면
// "높은 편"이 화면마다 다른 뜻이 된다. 정규화 0-100 값 기준이다.
...
export default function AttachmentStyleTest({ locale: rawLocale = "ko" }: Props) {
  ...
  // t.level·t.descriptions 는 6 로케일로 이미 쓰여 있었는데 렌더된 적이 없었다.
  // 두 카드가 대신 같은 RESPONSE_POSITION 문장을 반복했고, 그래서 결과가
  // 응답자마다 달라지지 않았다. 여기서 실제 수준을 쓴다.
  const anxietyLevel = attachmentLevel(result.anxiety);
  const avoidanceLevel = attachmentLevel(result.avoidance);
  const dimensions: { id: AttachmentDimension; label: string; value: number; level: "low" | "medium" | "high" }[] = [
    { id: "anxiety", label: t.anxiety, value: result.anxiety, level: anxietyLevel },
    { id: "avoidance", label: t.avoidance, value: result.avoidance, level: avoidanceLevel },
  ];
  return <div className="space-y-6">
    ...
    <div className="rounded-xl border border-green-200 bg-green-50 p-5 text-green-950">
