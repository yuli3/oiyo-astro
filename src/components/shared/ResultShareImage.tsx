...
export default function ResultShareImage({ title, level, score, color, icon, locale = 'ko' }: Props) {
  ...
  return (
    <button
      ...
      disabled={busy}
      aria-label={label}
      className="w-full rounded-xl border border-green-200 bg-surface-subtle text-green-700 px-4 py-2.5 text-sm font-bold hover:bg-green-100 transition-colors disabled:opacity-60"
    >
      {label}
    </button>
  )
