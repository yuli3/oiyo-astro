...
const BLOOD_TYPE_NAMES: Record<Locale, Record<BloodType, string>> = {
  ko: { A: "A형", B: "B형", O: "O형", AB: "AB형" },
  en: { A: "Type A", B: "Type B", O: "Type O", AB: "Type AB" },
  ja: { A: "A型", B: "B型", O: "O型", AB: "AB型" },
  zh: { A: "A型", B: "B型", O: "O型", AB: "AB型" },
  fr: { A: "Groupe A", B: "Groupe B", O: "Groupe O", AB: "Groupe AB" },
  es: { A: "Tipo A", B: "Tipo B", O: "Tipo O", AB: "Tipo AB" },
};
...
interface CategoryTexts {
  overall: Record<BloodType, Record<Tier, string[]>>;
  love: Record<BloodType, Record<Tier, string[]>>;
  money: Record<BloodType, Record<Tier, string[]>>;
  health: Record<BloodType, Record<Tier, string[]>>;
}

const BLOOD_TYPE_TEXTS: Record<Locale, CategoryTexts> = (() => {
  type BTPool = Record<BloodType, Record<Tier, string[]>>;

  function makePool(fn: (bt: BloodType, tier: Tier) => string[]): BTPool {
    const pool = {} as BTPool;
    for (const bt of BLOOD_TYPES) {
      pool[bt] = {} as Record<Tier, string[]>;
      for (const tier of ["great", "good", "mid", "low", "poor"] as Tier[]) {
        pool[bt][tier] = fn(bt, tier);
      }
    }
    return pool;
  }
  ...
  const koOverall = makePool((bt, tier) => {
    const name = BLOOD_TYPE_NAMES.ko[bt];
    const traits: Record<BloodType, string> = {
      A: "꼼꼼하고 성실한",
      B: "독창적이고 자유로운",
      O: "열정적이고 사교적인",
      AB: "창의적이고 분석적인",
    ...
    const map: Record<Tier, string[]> = {
      great: [
        `${name}의 ${t} 에너지가 최고조입니다! 오늘은 무엇이든 뜻대로 풀립니다.`,
        `행운이 가득한 하루입니다. ${name}의 장점이 빛을 발하는 날입니다.`,
        `오늘은 최상의 컨디션으로 큰 성과를 거둘 수 있습니다.`,
        `${name}을(를) 응원하는 기운이 넘칩니다. 과감하게 도전하세요!`,
        `모든 일이 순조롭게 풀리는 길운의 하루입니다.`,
      ],
      good: [
  ...
  const koLove = makePool((bt, tier) => {
    const name = BLOOD_TYPE_NAMES.ko[bt];
    const loveTraits: Record<BloodType, string> = {
      A: "섬세하고 배려 깊은",
      B: "자유롭고 개성 넘치는",
      O: "열정적이고 다가가기 쉬운",
      AB: "신비롭고 매력적인",
    };
    const t = loveTraits[bt];
    const map: Record<Tier, string[]> = {
      great: [
        `${name}의 ${t} 매력이 오늘 최고조입니다! 고백이나 데이트를 계획해 보세요.`,
        `사랑하는 사람과 특별한 순간을 나누기에 완벽한 날입니다.`,
        `새로운 인연이 찾아올 가능성이 높습니다. 주변을 잘 살펴보세요.`,
        `연인과의 깊은 유대감이 형성되는 날입니다. 진심을 표현하세요.`,
        `${name}의 애정운이 최고조! 관계가 한층 더 깊어집니다.`,
      ],
      good: [
  ...
  const koMoney = makePool((bt, tier) => {
    const name = BLOOD_TYPE_NAMES.ko[bt];
    const map: Record<Tier, string[]> = {
      great: [
  ...
  const koHealth = makePool((bt, tier) => {
    const name = BLOOD_TYPE_NAMES.ko[bt];
    const map: Record<Tier, string[]> = {
      great: [
  ...
  // ── English ──
  const enOverall = makePool((bt, tier) => {
    const name = BLOOD_TYPE_NAMES.en[bt];
    const traits: Record<BloodType, string> = {
      A: "meticulous and cooperative",
      B: "creative and free-spirited",
      O: "passionate and sociable",
      AB: "analytical and dual-natured",
    };
    const t = traits[bt];
    const map: Record<Tier, string[]> = {
      great: [
        `${name}'s ${t} energy is at its peak today! Everything flows in your favor.`,
        `Lucky energy fills your day. Your best qualities shine brightly.`,
        `Today is your day — big wins are within reach for ${name}.`,
        `Cosmic forces back you up. Step forward with confidence.`,
        `A peak day for ${name}. Go all-in on what matters most.`,
      ],
      good: [
      ...
      poor: [
        `${name}, the energy currents are rough today. Play it safe.`,
  ...
  const enMoney = makePool((bt, tier) => {
    const name = BLOOD_TYPE_NAMES.en[bt];
    const map: Record<Tier, string[]> = {
      great: [
  ...
  const enHealth = makePool((bt, tier) => {
    const name = BLOOD_TYPE_NAMES.en[bt];
    const map: Record<Tier, string[]> = {
      great: [
  ...
  const jaTpl: Record<string, Record<Tier, (n: string) => string[]>> = {
    overall: {
      great: (n) => [`${n}に最高の一日が訪れました！積極的に動いて大きな成果を掴みましょう。`,`幸運のエネルギーが満ち溢れています。${n}の魅力が輝く日です。`,`星の加護を受けた日です。挑戦を恐れず前に進みましょう。`,`${n}を応援する力が最大です。大きな一歩を踏み出す時。`,`何事もうまくいく予感の一日です。自信を持って臨みましょう。`],
  ...
  function makeJaPool(cat: string): BTPool {
    return makePool((bt, tier) => (jaTpl[cat] as Record<Tier, (n: string) => string[]>)[tier](BLOOD_TYPE_NAMES.ja[bt]));
  }

  // ── Chinese Simplified ──
  const zhTpl: Record<string, Record<Tier, (n: string) => string[]>> = {
    overall: {
      great: (n) => [`${n}今天运势达到顶峰！积极行动，收获丰硕成果。`,`超凡能量环绕你，你最好的状态今天闪耀。`,`这是${n}的高光时刻，相信直觉，勇往直前。`,`宇宙力量与你同在，全力投入最重要的事情。`,`重大胜利触手可及，抓住每一个机会。`],
      ...
      poor: (n) => [`${n}，今天能量气流汹涌，稳健行事。`,`避免冒险，保护你已建立的一切。`,`挑战来临，但这也会过去，保持韧性。`,`今天适合安静反思，而非大胆行动，充电蓄能。`,`今日运势低，休息、规划、准备迎接更好的日子。`],
    },
    love: {
  ...
  function makeZhPool(cat: string): BTPool {
    return makePool((bt, tier) => (zhTpl[cat] as Record<Tier, (n: string) => string[]>)[tier](BLOOD_TYPE_NAMES.zh[bt]));
  }

  // ── French ──
  const frTpl: Record<string, Record<Tier, (n: string) => string[]>> = {
    overall: {
      great: (n) => [`${n}, aujourd'hui est votre jour de gloire ! Foncez et récoltez les fruits.`,`Une énergie exceptionnelle vous entoure. Votre meilleur vous brille.`,`Un moment de sommet pour ${n}. Faites confiance à vos instincts.`,`Les forces cosmiques sont de votre côté. Investissez là où ça compte.`,`De grandes victoires sont à portée pour ${n} aujourd'hui.`],
  ...
  function makeFrPool(cat: string): BTPool {
    return makePool((bt, tier) => (frTpl[cat] as Record<Tier, (n: string) => string[]>)[tier](BLOOD_TYPE_NAMES.fr[bt]));
  }

  // ── Spanish ──
  const esTpl: Record<string, Record<Tier, (n: string) => string[]>> = {
    overall: {
      great: (n) => [`${n}, ¡hoy es tu día de gloria! Lánzate y recoge los frutos.`,`Una energía excepcional te rodea. Tu mejor yo brilla intensamente.`,`Un momento cumbre para ${n}. Confía en tus instintos y avanza.`,`Las fuerzas cósmicas están de tu lado. Invierte en lo que más importa.`,`Las grandes victorias están al alcance para ${n} hoy.`],
      ...
      poor: (n) => [`${n}, las corrientes de energía son agitadas hoy. Juega a lo seguro.`,`Evita las aventuras arriesgadas y protege lo que has construido.`,`Los desafíos llegan, pero esto también pasará. Mantente resiliente.`,`Un día de reflexión silenciosa, no de acción audaz. Recarga tu energía.`,`Baja fortuna hoy — descansa, planifica y prepárate para días mejores.`],
    },
    love: {
  ...
  function makeEsPool(cat: string): BTPool {
    return makePool((bt, tier) => (esTpl[cat] as Record<Tier, (n: string) => string[]>)[tier](BLOOD_TYPE_NAMES.es[bt]));
  }
...
const UI: Record<Locale, {
  ...
  en: {
    title: "Blood Type Daily Fortune",
    subtitle: "Select your blood type to reveal today's fortune",
    selectLabel: "My Blood Type",
    resultDate: "Today's Fortune",
    overall: "Overall",
    love: "Love",
    money: "Money",
    health: "Health",
    ...
    resetBtn: "Change Type",
  },
  ja: {
    title: "血液型今日の運勢",
    subtitle: "血液型を選んで今日の運勢を確認しましょう",
    selectLabel: "私の血液型",
    resultDate: "今日の運勢",
    overall: "総合運",
    love: "恋愛運",
  ...
  fr: {
    title: "Fortune Quotidienne par Groupe Sanguin",
    subtitle: "Sélectionnez votre groupe sanguin pour révéler votre fortune du jour",
    selectLabel: "Mon Groupe Sanguin",
    resultDate: "Fortune d'Aujourd'hui",
    overall: "Général",
    love: "Amour",
    money: "Argent",
    health: "Santé",
    ...
    resetBtn: "Changer de Groupe",
  },
  es: {
    title: "Fortuna Diaria por Tipo de Sangre",
    subtitle: "Selecciona tu tipo de sangre para revelar tu fortuna de hoy",
    selectLabel: "Mi Tipo de Sangre",
    resultDate: "Fortuna de Hoy",
    overall: "General",
    love: "Amor",
    money: "Dinero",
    health: "Salud",
...
function generateFortune(bloodType: BloodType, locale: Locale): FortuneResult {
  const btIdx = BLOOD_TYPES.indexOf(bloodType);
  const rand = seededRand(todaySeed(btIdx));
  ...
  const texts = BLOOD_TYPE_TEXTS[locale];
  ...
  return {
    bloodType,
    overall: { score: overallScore, text: pickText(texts.overall[bloodType], overallScore) },
    love: { score: loveScore, text: pickText(texts.love[bloodType], loveScore) },
    money: { score: moneyScore, text: pickText(texts.money[bloodType], moneyScore) },
    health: { score: healthScore, text: pickText(texts.health[bloodType], healthScore) },
...
export default function BloodTypeFortune({ locale }: Props) {
      ...
      {!result ? (
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {BLOOD_TYPES.map((bt) => (
              ...
              >
                ...
                <span className="mt-2 text-sm font-semibold text-gray-800">{names[bt]}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-red-200 bg-gradient-to-br from-red-50 to-rose-50 p-5">
            <h3 className="mb-3 text-center font-bold text-red-800">✨ {ui.luckySection}</h3>
            <div className="flex justify-center gap-10">
              <div className="text-center">
                <p className="text-xs text-red-500 mb-1">{ui.luckyColor}</p>
                <p className="text-lg font-semibold text-red-700">{result.luckyColor}</p>
              </div>
              <div className="text-center">
                <p className="text-xs text-red-500 mb-1">{ui.luckyNumber}</p>
                <p className="text-2xl font-bold text-red-700">{result.luckyNumber}</p>
              </div>
            </div>
          </div>
