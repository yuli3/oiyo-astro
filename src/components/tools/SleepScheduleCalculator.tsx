...
export default function SleepScheduleCalculator() {
  ...
  return <section className="rounded-2xl border border-green-100 bg-green-50 p-5" aria-labelledby="sleep-calculator-title">
    ...
    <div className="mt-4 flex flex-wrap items-end gap-3"><label className="grid gap-1 font-semibold text-foreground">{label}<input aria-invalid={invalid || undefined} className="rounded-lg border border-green-200 bg-card px-3 py-2" type="time" value={value} onChange={(event) => setValue(event.target.value)} /></label><button className="rounded-lg bg-green-700 px-4 py-2 font-semibold text-white" type="button" onClick={() => setSubmitted(true)}>Calculer</button></div>
