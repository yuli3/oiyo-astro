...
export default function SleepScheduleCalculator() {
  ...
  return <section className="rounded-2xl border border-green-100 bg-green-50 p-5" aria-labelledby="sleep-calculator-title">
    <h1 id="sleep-calculator-title" className="text-2xl font-bold text-green-950">Calculateur de sommeil : heure de coucher et de réveil</h1>
    ...
    <fieldset className="mt-4"><legend className="font-semibold text-green-950">Choisissez un repère</legend><div className="mt-2 flex flex-wrap gap-3">
    ...
    <div className="mt-4 flex flex-wrap items-end gap-3"><label className="grid gap-1 font-semibold text-green-950">{label}<input aria-invalid={invalid || undefined} className="rounded-lg border border-green-200 bg-white px-3 py-2" type="time" value={value} onChange={(event) => setValue(event.target.value)} /></label><button className="rounded-lg bg-green-700 px-4 py-2 font-semibold text-white" type="button" onClick={() => setSubmitted(true)}>Calculer</button></div>
    {invalid && <p className="mt-3 text-sm text-red-700" role="alert">Saisissez une heure valide au format HH:mm.</p>}
    {schedule && <div className="mt-5" aria-live="polite"><h2 className="font-semibold text-green-950">{resultLabel}</h2><ul className="mt-2 grid gap-2 sm:grid-cols-5">{schedule.map((entry, index) => <li className="rounded-lg bg-white p-3 text-center" key={entry.duration}><span className="block text-xs text-green-700">environ {DURATIONS[index]}</span><strong className="text-lg text-green-950">{entry.time}</strong></li>)}</ul></div>}
