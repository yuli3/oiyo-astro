...
function AttachmentBar({ label, value }: { label: string; value: number }) {
  ...
  return (
    <div>
      ...
      <div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-violet-600" style={{ width: `${value}%` }} /></div>
    </div>
  );
}
  ...
  return (
    <section className="mt-8 rounded-[28px] border border-green-100 bg-green-50/40 p-4 shadow-sm">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        ...
        {groups.attachment.length === ATTACHMENT.length && <article className="rounded-2xl border border-violet-100 bg-white p-4">
          <p className="text-[10px] font-black uppercase tracking-wider text-violet-600">{t.attachmentRole}</p>
