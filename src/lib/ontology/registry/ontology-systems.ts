...
// (deep dive); this catalog drives the crawlable lane grid + input gating.
//
// 2026-08-26 (세운 지시): each system's own subpage explains it natively now —
// dropped the wiki-definition explanation bridge (wikiSlug) entirely rather
// than sending visitors to wiki.oiyo.net from here.
...
const INNATE: OntologySystem[] = [
  { id: 'saju', lane: 'innate', emoji: '🔮', requires: 'birthDate', href: (l) => `/${l}/saju/calculator`, name: L('사주팔자', 'Saju (Four Pillars)', '四柱推命', '四柱八字', 'Saju (Quatre Piliers)', 'Saju (Cuatro Pilares)') },
...
const TEST: OntologySystem[] = [
  ...
  { id: 'friendship-style', lane: 'test', emoji: '🫂', requires: 'testResult', testId: 'friendship-style', href: (l) => `/${l}/friendship-style-test/`, name: L('우정 스타일', 'Friendship Style', '友情スタイル', '友情风格', 'Style d’amitié', 'Estilo de amistad') },
  ...
  { id: 'spending-habits', lane: 'test', emoji: '🛍️', requires: 'testResult', testId: 'spending-habits', href: (l) => `/${l}/spending-habits-test/`, name: L('소비 습관', 'Spending Habits', '消費習慣', '消费习惯', 'Habitudes de dépense', 'Hábitos de consumo') },
  { id: 'country-match', lane: 'test', emoji: '🗺️', requires: 'testResult', testId: 'country-match', href: (l) => `/${l}/country/match/`, name: L('나에게 맞는 나라', 'Country Match', '合う国', '适合的国家', 'Pays idéal', 'País ideal') },
