...
// 크기: raw 2.4MB · gzip 964KB · 로컬 최고압축 brotli 660KB.
// **실제 전송량은 약 957KB 다** — Cloudflare 는 CPU 를 아끼려 낮은 brotli 품질로
// 압축하므로 최고압축 수치를 그대로 인용하면 안 된다(2026-09-01 라이브 실측).
// public/_headers 의 `/data/*` 규칙으로 일주일 브라우저 캐시를 건다. 그 규칙이
// 없으면 기본 `max-age=0` 때문에 검색을 열 때마다 조건부 재요청이 왕복한다. 임계를 인구 5만으로 올리면 3분의 1로 줄지만 국가
